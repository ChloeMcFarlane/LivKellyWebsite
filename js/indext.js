...
document.addEventListener('DOMContentLoaded',()=>{const lazyLoader=new LazyImageLoader();const scrollNav=new ScrollNavigation('scrollContainer','scrollLeft','scrollRight');document.addEventListener('keydown',(e)=>{const container=document.getElementById('scrollContainer');if(!container)return;if(e.key==='ArrowLeft'){e.preventDefault();document.getElementById('scrollLeft').click()}else if(e.key==='ArrowRight'){e.preventDefault();document.getElementById('scrollRight').click()}});const firstImages=document.querySelectorAll('.lazy-image');if(firstImages.length>0){lazyLoader.loadImage(firstImages[0]);if(firstImages.length>1){lazyLoader.loadImage(firstImages[1])}}});
let touchStartX=0;let touchEndX=0;document.addEventListener('DOMContentLoaded',()=>{const container=document.getElementById('scrollContainer');if(!container)return;container.addEventListener('touchstart',(e)=>{touchStartX=e.changedTouches[0].screenX},{passive:true});container.addEventListener('touchend',(e)=>{touchEndX=e.changedTouches[0].screenX;handleSwipe()},{passive:true});function handleSwipe(){const swipeThreshold=50;const diff=touchStartX-touchEndX;if(Math.abs(diff)>swipeThreshold){if(diff>0){document.getElementById('scrollRight')?.click()}else{document.getElementById('scrollLeft')?.click()}}}});